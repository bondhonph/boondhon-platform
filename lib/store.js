...
async function loadDataFromDrive() {
  // If memory cache exists and has keys, use it
  if (global.__boondhonConversationsCache !== null && Object.keys(global.__boondhonConversationsCache).length > 0) {
    return global.__boondhonConversationsCache;
  }

  ensureDirectory();

  // Try fetching from Google Drive Web App URL
  try {
    const res = await fetch(DRIVE_SYNC_URL, { redirect: 'follow' });
    if (res.ok) {
      let text = await res.text();
      if (text) {
        let driveData = JSON.parse(text);
        if (typeof driveData === 'string') {
          driveData = JSON.parse(driveData);
        }
  ...
  if (fs.existsSync(FILE_PATH)) {
    try {
      ...
      if (localData && Object.keys(localData).length > 0) {
        global.__boondhonConversationsCache = localData;
        return global.__boondhonConversationsCache;
      }
    } catch (e) {
...
function saveData(data) {
  global.__boondhonConversationsCache = data;
  saveLocalFile(data);

...
export function buildPriceTableText(category, fmt = bngDigits) {
  ...
  const lines = getReferenceTable(category)
    ...
    .join('\n');
  return `${cat.label} কালেকশনের রেট (মিনিমাম ৫০ পিস):\n${lines}`;
}

export function buildBothCategoriesPriceText(fmt = bngDigits) {
  return (
    `আমাদের বিয়ের কার্ডের অফিশিয়াল দামের তালিকা: 🌸\n` +
    `(মিনিমাম অর্ডার ৫০ পিস থেকে শুরু)\n\n` +
    `${buildPriceTableText('affordable', fmt)}\n\n` +
    `${buildPriceTableText('premium', fmt)}\n\n` +
    `🎁 ২০০+ পিসে পাচ্ছেন ১টি আকর্ষণীয় নিকাহনামা একদম ফ্রি উপহার!`
  );
}
...
export function buildPricingBlurbForAI(fmt = bngDigits) {
  return [
    `• অফিশিয়াল মিনিমাম অর্ডার: ৫০ পিস থেকে শুরু। সাধারণ দামের আলোচনায় সবসময় ৫০, ১০০ ও ২০০ পিসের দাম জানাবে।`,
    ...
    `৩. অল্প পরিমাণ (১-৪৯ পিস — শুধু যদি কাস্টমার নিজে থেকে ৫০ পিসের কম চায়, তখনই কেবল বলবে): ১-৫ পিস ১,০০০৳, ৬-১০ পিস ১,৫০০৳, ১১-৪৯ পিস ৭৫৳/পিস। তবে ৫০ পিস নেওয়াই সবচেয়ে লাভজনক ও সাশ্রয়ী।`,
  ].join('\n');
}

/** Low-quantity (<50 pcs) price message: explains min 50 pcs first with 50/100/200 rates, then gives requested low qty rate. */
export function buildLowQtyPriceText(qty, category = 'affordable', fmt = bngDigits) {
  const total = getLowQtyTotal(qty, category);
  let rateMsg;
  if (qty <= 5) {
    rateMsg = `• ${fmt(qty)} পিসের মোট দাম: ${fmt(total.toLocaleString('en-IN'))}৳ (ফিক্সড মেকিং ও ডাইস চার্জ সহ)`;
  } else if (qty <= 10) {
    rateMsg = `• ${fmt(qty)} পিসের মোট দাম: ${fmt(total.toLocaleString('en-IN'))}৳ (ফিক্সড চার্জ সহ)`;
  } else {
    rateMsg = `• ${fmt(qty)} পিসের মোট দাম: ${fmt(total.toLocaleString('en-IN'))}৳ (পিস প্রতি ৭৫৳)`;
  }

  return (
    `আমাদের বিয়ের কার্ডের অফিশিয়াল মিনিমাম অর্ডার ৫০ পিস থেকে শুরু। কারণ ৫০ বা ১০০ পিস বানালে ডাইস, প্লেট ও মেকিং চার্জ ভাগ হয়ে পিস প্রতি খরচ অনেক কম পড়ে: 🌸\n\n` +
    `💚 সাশ্রয়ী (Affordable):\n` +
    `• ৫০ পিস: ২,৭৫০৳ (৫৫৳/পিস)\n` +
    ...
    `━━━━━━━━━━━━━━━━━━━━\n` +
    `📦 আপনার যদি নিতান্তই ${fmt(qty)} পিস প্রয়োজন হয়:\n` +
    `${rateMsg}\n\n` +
    `💡 পরামর্শ: ৫০ পিস নিলে প্রতি পিস অনেক বেশি সাশ্রয়ী পড়বে! আপনি কত পিস করতে চান বলুন? 😊`
  );
}

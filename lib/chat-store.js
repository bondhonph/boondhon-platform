...
export function setSelectedCard(phone, cardInfo) {
  const store = readStore();
  const cleanPhone = String(phone).replace(/\D/g, '');
  if (!store[cleanPhone]) {
    store[cleanPhone] = {
  ...
  store[cleanPhone].selectedCard = cardInfo;
  store[cleanPhone].lastUpdated = Date.now();
  writeStore(store);
  return store[cleanPhone];
}

export function getSelectedCard(phone) {
  const store = readStore();
  const cleanPhone = String(phone).replace(/\D/g, '');
  return store[cleanPhone]?.selectedCard || null;
}

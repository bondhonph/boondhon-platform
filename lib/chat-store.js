...
const redisBackend = {
  ...
  async setTakeover(phone, active, durationSeconds = TAKEOVER_TTL_SECONDS) {
    const redis = getRedis();
    if (!redis) {
      return fileBackend.setTakeover(phone, active, durationSeconds);
    }
    const key = TAKEOVER_PREFIX + phone;
    try {
      if (active) {
        await redis.set(key, Date.now(), { ex: durationSeconds });
      } else {
        await redis.del(key);
      }
    } catch (err) {
  ...
  async isTakeoverActive(phone) {
    const redis = getRedis();
    if (!redis) {
      return fileBackend.isTakeoverActive(phone);
    }
    try {
      const val = await redis.get(TAKEOVER_PREFIX + phone);
      return Boolean(val);
    } catch (err) {
      console.error(`[chat-store] Redis isTakeoverActive error for ${phone}:`, err.message);
      return fileBackend.isTakeoverActive(phone);
    }
  },
};
...
const fileBackend = {
  ...
  async setTakeover(phone, active) {
    // Handled in record.humanTakeover
  },
  async isTakeoverActive(phone) {
    const rec = await this.getRecord(phone);
    if (!rec || !rec.humanTakeover) return false;
    return (Date.now() - (rec.lastAdminReplyTime || 0)) < (TAKEOVER_TTL_SECONDS * 1000);
  },
};
...
export async function setHumanTakeover(phone, status) {
  ...
  await backend.setTakeover(cleanPhone, Boolean(status));
  return save(cleanPhone, record);
}

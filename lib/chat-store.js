...
async function loadOrInit(phone, extraName) {
  ...
  if (existing) {
    ...
    if (existing.selectedTier === undefined) existing.selectedTier = existing.currentCategory || null;
    if (existing.selectedQuantity === undefined) existing.selectedQuantity = existing.order?.quantity || null;
    if (existing.priceInquiryCount === undefined) existing.priceInquiryCount = 0;
    return existing;
  }
...
export async function setCurrentCategory(phone, category) {
  ...
  record.selectedTier = category;
  return save(cleanPhone, record);
}

export async function getCurrentCategory(phone) {
  const record = await getConversation(phone);
  return record?.selectedTier || record?.currentCategory || null;
}

export async function setSelectedTier(phone, tier) {
  return setCurrentCategory(phone, tier);
}

export async function getSelectedTier(phone) {
  return getCurrentCategory(phone);
}

export async function setSelectedQuantity(phone, quantity) {
  const cleanPhone = cleanPhoneOf(phone);
  const record = await loadOrInit(cleanPhone);
  record.selectedQuantity = quantity;
  if (!record.order) record.order = blankRecord(cleanPhone).order;
  record.order.quantity = quantity;
  return save(cleanPhone, record);
}

export async function getSelectedQuantity(phone) {
  const record = await getConversation(phone);
  return record?.selectedQuantity ?? record?.order?.quantity ?? null;
}

export async function incrementPriceInquiryCount(phone) {
  const cleanPhone = cleanPhoneOf(phone);
  const record = await loadOrInit(cleanPhone);
  record.priceInquiryCount = (record.priceInquiryCount || 0) + 1;
  await save(cleanPhone, record);
  return record.priceInquiryCount;
}

export async function getPriceInquiryCount(phone) {
  const record = await getConversation(phone);
  return record?.priceInquiryCount || 0;
}

export async function resetPriceInquiryCount(phone) {
  const cleanPhone = cleanPhoneOf(phone);
  const record = await loadOrInit(cleanPhone);
  record.priceInquiryCount = 0;
  return save(cleanPhone, record);
}
...
export async function updateOrder(phone, partialFields) {
  ...
  if (partialFields && partialFields.quantity !== undefined) {
    record.selectedQuantity = partialFields.quantity;
  }
  return save(cleanPhone, record);
}
